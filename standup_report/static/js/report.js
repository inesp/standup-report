...
showCreated.addEventListener('change', () => {
  document.querySelectorAll('.activity-item.item-created-issue').forEach(el => {
...
function addToIgnoredList(itemType, itemId, itemTitle) {
  ...
  p.querySelector('.unignore-btn').addEventListener('click', (e) => handleIgnoreClick(e, 'unignore'));
  list.appendChild(p);
}
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (action === 'ignore') {
        ...
        showMessage('Item unignored. Reload to see it in Done/Next. (We have to fetch all of its data again from the remote.)', 'info');
      }
    } else {
